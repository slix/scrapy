import re
import time
from http.cookiejar import Cookie, CookieJar as _CookieJar, DefaultCookiePolicy
...
class CookieJar:
    def __init__(self, policy: None=None, check_expired_frequency: int=10000) -> None:
    ...
    def make_cookies(self, response: Response, request: Request) -> List[Cookie]:
        wreq = WrappedRequest(request)
        wrsp = WrappedResponse(response)
...
class WrappedResponse:
    ...
    def info(self) -> WrappedResponse:
        return self

    def get_all(self, name: str, default: Optional[List[Any]]=None) -> List[Union[Any, str]]:
